...
const QRCodeLink = ({ link }) => {
  ...
  useEffect(() => {
    if (link) {
      ...
      setURL(`${currentDomain}/r/${link.shortLink}`)
    }
  }, [link])
    ...
    if (QRCodeRef.current) {
      toPng(QRCodeRef.current)
        .then((dataURL) => {
          ...
          vLink.download = `QR-Code_${link._id}.png`
          vLink.href = dataURL
          vLink.click()
        })
        .catch((error) => {
  ...
  return (
    <div className={`flex flex-col items-center justify-center gap-2`}>
      <div className={`rounded-box overflow-hidden shadow-lg min-w-48 min-h-48 ${!link ? 'skeleton ' : ''}`} ref={QRCodeRef}>
        {
          link &&
          <QRCodeSVG
            value={URL}
